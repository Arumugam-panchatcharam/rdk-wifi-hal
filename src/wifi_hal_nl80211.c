...
/*  VLAN IMPL
 * 1. Create wifi interface "wifi0" - nl80211_create_interface/nl80211_update_interface
 * 2. Create VLAN link reference "wifi0.12"
 * 3. Enslave VLAN interface to the bridge
 * 4. configure hostapd with VLAN interface name
 */
/*
...
int nl80211_add_vlan(const char *if_name, int vid, const char *vlan_if_name)
{
	...
	wpa_printf(MSG_ERROR, "VLAN: vlan_add(if_name=%s, vid=%d, "
		   "vlan_if_name=%s)\n", if_name, vid, vlan_if_name);

    wifi_hal_info_print("%s:%d: [TRAFFIC_SEP] VLAN: vlan_add(if_name=%s, vid=%d, vlan_if_name=%s)\n", __func__, __LINE__, if_name, vid, vlan_if_name);

    if ((os_strlen(if_name) + 1) > IFNAMSIZ) {
		wpa_printf(MSG_ERROR, "Interface name too long: '%s'",
			   if_name);
		return -1;
	}
...
int nl80211_remove_vlan(const char *if_name)
{
	...
	wpa_printf(MSG_ERROR, "VLAN: vlan_rem(if_name=%s)", if_name);
